      ...
      {!compact && moveSummary.length ? (
        <div className="mt-2 grid gap-2 sm:grid-cols-2">
          {moveSummary.map((entry, moveIndex) => (
            <div
              className="panel-dark-soft rounded-md border border-slate-700/50 px-3 py-1.5 h-[88px] flex flex-col overflow-hidden"
              key={`${member.id}-${moveIndex}`}
            >
              <p className="text-sm font-semibold text-slate-100">
                {entry.move} <span className="text-slate-400 text-xs">({toTitleCase(entry.type)})</span>
              </p>
              <div className="mt-1.5 h-[24px] flex flex-wrap gap-1 content-start overflow-hidden">
